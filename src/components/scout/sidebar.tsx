...
const scoutSidebarMenuItems: MenuItem[] = [
  {
    id: 1,
    name: "Players",
    icon: "fas fa-users",
...
function MenuItems({ setOpen }: MenuItemsProps) {
  ...
  const scoutName = currentProfile
    ? `${currentProfile.firstName || ""} ${currentProfile.lastName || ""}`.trim()
    : "Loading...";

  const scoutOrganisation = currentProfile?.organisation || "Scout";
  ...
  return (
    <nav className="flex flex-col gap-6 p-4 w-full h-full overflow-y-auto">
      ...
      <div className="flex flex-col gap-3 w-full px-4">
        {scoutSidebarMenuItems.map((menuItem) => {
          const isActive =
...
function ScoutSideBar({ open, setOpen }: ScoutSideBarProps) {
