      ...
      <main className="flex-1 p-6 dark:bg-gray-900 dark:text-white">
        ...
        <div className="min-h-screen bg-gray-100 p-6 mt-4 rounded-xl dark:bg-slate-800">
          ...
          <div className="flex flex-wrap gap-4 justify-start mb-6">
            {["Profession", "City", "Country", "Gender", "Language"].map(
              (filter) => (
                <select
                  key={filter}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {experts.map((expert, index) => (
              <div
                key={index}
                className="bg-white shadow-md rounded-lg p-4 w-80 dark:bg-slate-600 dark:text-white"
              >
                ...
                <div className="mt-4 text-center">
                  ...
                  <div className="flex mt-4">
                    <span className="text-yellow-500 text-lg text-left">
                      ⭐
                    </span>
                    <span className="ml-1 text-gray-700 text-left dark:text-white">
                      {expert.rating}/5
                    </span>
                    <p className="ml-28 text-red-600 text-xl font-bold text-right">
                      {expert.reviews}+
                    </p>
                  </div>
                  <button
                    className="mt-4 bg-red-600 text-white px-4 py-2 w-full rounded-xl text-lg"
                    onClick={() => navigate("/exdetails")}
                  >
                    View Profile
                  </button>
                </div>
