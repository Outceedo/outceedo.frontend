    ...
    try {
      ...
      if (err?.response?.status === 403) {
        Swal.fire({
          ...
          html:
            "Reports older than 7 days are locked on the Free plan.<br /><br />" +
            `<button id="plans-btn" class="swal2-confirm swal2-styled" style="background:#EF4444;color:#fff;padding:0.5em 2em;margin-top:1em;">View Plans</button>`,
          showConfirmButton: false,
          didOpen: () => {
            const btn = document.getElementById("plans-btn");
            if (btn) btn.onclick = () => { Swal.close(); navigate("/plans"); };
          },
        });
      }
      return false;
