    ...
    <div key={index} className="flex items-center bg-pink-100 p-2 rounded-lg shadow gap-4 mt-3 dark:bg-slate-500 dark:text-white">
      ...
      <div className="flex items-center justify-between w-full">
        <div>
          ...
          <p className="text-gray-500 text-sm dark:text-white">10+ years ⭐ 3.5/5</p>
        </div>
        <FontAwesomeIcon icon={faArrowUpRightFromSquare} className="text-gray-500 dark:text-white ml-4" />
      </div>
    </div>
  ))}
</div>
        ...
        <div className="bg-yellow-100 p-4 rounded-lg shadow mt-6 dark:bg-slate-600">
          <h2 className="text-gray-700 font-bold dark:text-white">Notifications</h2>
          <div className="mt-3 space-y-2 ">
            {[1, 2, 3].map((_, i) => (
              <div key={i} className="p-3 bg-white rounded-md shadow dark:bg-slate-300 dark:text-black">
                Notification {i + 1}
              </div>
            ))}
          </div>
